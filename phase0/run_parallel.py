...
def main() -> int:
    ...
    if "${LR}" in text:
        if a.lr_scale is None:
            sys.exit(f"{runner.name} uses ${{LR}}; pass --lr-scale")
        m = re.search(r'VALID="([^"]*)"', text)
        if m and a.lr_scale not in m.group(1).split():
            sys.exit(f"lr_scale {a.lr_scale!r} is not in the runner's VALID list ({m.group(1)}); "
    ...
    for line in text.splitlines():
        ...
        if a.lr_scale is not None:
            line = line.replace("${LR}", a.lr_scale)
        ...
        ckdir = Path(load_cfg(cfg_path)["checkpoint"]["dir"] + suffix)
        final = (ckdir if ckdir.is_absolute() else cwd / ckdir) / "final.pt"
        runs.append({"name": cfg_path.stem + suffix, "cmd": toks, "final": final})
    if not runs:
