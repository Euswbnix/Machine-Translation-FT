...
\paragraph{Single-checkpoint comparison.} For SFT, where the run is too short to produce 5 save-interval-spaced checkpoints, we evaluate the final single checkpoint. We flag this unit-mismatch in Table~\ref{tab:sft_results} and note that it inflates the apparent SFT degradation by approximately the averaging gain (0.3--0.5 BLEU).

\subsection{Compute budget}
The full study --- five headline pretraining runs (en-fr Base v1.1, Big v1.1, Base v2, Big v2; en-de Base) plus two SFT runs plus 13.8~h of CometKiwi scoring --- totals $\sim$~30 GPU-hours on the 5090.
