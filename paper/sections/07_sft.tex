...
\textbf{Unit-of-comparison reminder.} The pretraining baselines we compare against are 5-checkpoint averages (Vaswani's averaging trick); the fine-tuning runs in this section are too short (6K steps) to produce 5 save-interval-spaced checkpoints, so their numbers are single final checkpoints. Averaging the fine-tuning runs would likely lift their BLEU by 0.2--0.4. Reported $\Delta$s in Table~\ref{tab:sft_results} therefore overstate the fine-tuning loss by approximately the averaging gain. The directional finding (monotonic degradation, Big-loses-more-than-Base) is unaffected.

BLEU monotonically decreased throughout fine-tuning, on both models, on both splits (Table~\ref{tab:sft_results}, Figure~\ref{fig:sft_curves}).
